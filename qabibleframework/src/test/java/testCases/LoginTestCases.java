package testCases;
import java.io.IOException;

import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;
import org.testng.Assert;
import org.testng.annotations.Test;

import base.BaseClass;
import elementRepository.DashboardPage;
import elementRepository.LoginPage;
import utilities.ExcelReadUtility;
import utilities.GeneralUtilities;

public class LoginTestCases extends BaseClass {
	
  @Test
  public void verifyLoginisWorkingWithValidCredentials() throws IOException
  {
	  LoginPage lp = new LoginPage(driver);
	  lp.inputUsername(ExcelReadUtility.readStringData(1, 1));
	  lp.inputPassword(ExcelReadUtility.readStringData(2, 2));
	  lp.clickLoginButton();
	  
	  DashboardPage dp = lp.clickLoginButton();
	  String actual = dp.getTextFromTitle();
	  String expected = "PAYROLL APPLICATION";
	  Assert.assertEquals(actual, expected,"Login feature is not working");
  }

  @Test
  public void verifyToolTpValue() {
	  LoginPage lp = new LoginPage(driver);
	  lp.inputUsername("carol");
	  lp.inputPassword("1q2w3e4r");
	  lp.clickLoginButton();
	  
	  DashboardPage dp = lp.clickLoginButton();
	  String actualToolTipValue = dp.getToolTipValue();
	  String expectedToolTipValue = "PAYROLL APPLICATION";
	  Assert.assertEquals(actualToolTipValue, expectedToolTipValue);
  }
  
}